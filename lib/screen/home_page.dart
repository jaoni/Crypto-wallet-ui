...
import '../widget/botton_navigation.dart';
import '../widget/portfolio.dart';
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            FadeInDownBig(child: const Center(child: Text('Current Wallet Ballance', 
            style: TextStyle(color: Colors.grey, fontSize: 18),))),
            const SizedBox(height: 5,),
            FadeInDownBig(child: Center(child: Text('\$3,293.45', 
            style: TextStyle(color:Colors.yellowAccent[700], fontSize: 45, fontWeight: FontWeight.bold, fontFamily: ''),))),
            const SizedBox(height: 25,),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                FadeInLeft(child:_sendReceive(context, title: 'Send', 
                icon: Icon(IconlyBroken.arrow_up, size: 30, color:Colors.grey[400]))),
                const SizedBox(width:20,),
                FadeInDown(child:_sendReceive(context, title: 'Receive', 
                icon: Icon(IconlyBroken.arrow_down, size: 30, color:Colors.grey[400]))),
                const SizedBox(width:20,),
                FadeInUp(child:_sendReceive(context, title: 'Buy', icon:
                Icon(IconlyBroken.bag_2, size: 30, color:Colors.grey[400]))),
                const SizedBox(width:20,),
                FadeInRight(child:_sendReceive(context, title: 'Swap', icon: 
                Icon(IconlyBroken.swap, size: 30, color:Colors.grey[400]))),
              ],
            ),
            const SizedBox(height: 25,),
